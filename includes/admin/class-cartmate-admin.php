...
if (
    isset( $_POST['cartmate_add_sequence'] ) ||
    isset( $_POST['cartmate_update_sequence_id'] ) ||
    isset( $_POST['cartmate_delete_sequence_id'] )
) {
...
protected function handle_sequences_crud() {
    ...
    // Only run on actual Sequences CRUD posts.
    $is_seq_action =
        isset( $_POST['cartmate_add_sequence'] ) ||
        isset( $_POST['cartmate_update_sequence_id'] ) ||
        isset( $_POST['cartmate_delete_sequence_id'] );
    ...
    // Nonce check (must exist in render_tab_sequences()).
    if (
        ! isset( $_POST['cartmate_sequences_nonce'] ) ||
        ! wp_verify_nonce(
            sanitize_text_field( wp_unslash( $_POST['cartmate_sequences_nonce'] ) ),
            'cartmate_sequences_crud'
        )
    ) {
        add_settings_error(
            'cartmate_messages',
            'cartmate_seq_bad_nonce',
            __( 'Security check failed. Please try again.', 'cartmate' ),
            'error'
        );
        return;
    }
    ...
    public function render_settings_page() {
        ...
        <div class="wrap cartmate-wrap">
            ...
            <form method="post" action="">
                ...
                <?php wp_nonce_field( 'cartmate_sequences_crud', 'cartmate_sequences_nonce' ); ?>
            ...
            <div class="cartmate-card">
                ...
                <div class="cartmate-grid" style="grid-template-columns: 1fr 1fr; gap: 16px;">
                    <div class="cartmate-card" style="margin:0;">
                        ...
                        <div class="cartmate-field">
<label for="sequence_delay_days"><?php esc_html_e( 'Delay (days after previous email)', 'cartmate' ); ?></label>
...
printf(
    ...
    esc_html__( '%d day(s) after previous email', 'cartmate' ),
    (int) $r['delay_days']
);
?>
                                        </span>
    ...
    protected function render_tab_sms( $sms_delay, $cs_username, $cs_api_key, $cs_sender, $sms_template, $test_phone, $sms_default_country, $sms_test_country ) {
        ...
        ?>
        <div class="cartmate-grid cartmate-grid--wide">
    <div class="cartmate-card cartmate-card--wide">
    ...
    protected function render_tab_status() {
        ?>
<div class="cartmate-grid cartmate-grid--page">
    <div class="cartmate-card cartmate-card--page">
