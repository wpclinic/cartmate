...
public static function normalize_phone_e164( string $raw_phone, string $country_code ): string {
    ...
    public static function send_test_sms( $phone ) {
        $phone = trim( $phone );
        if ( empty( $phone ) ) {
            return new \WP_Error( 'cartmate_test_no_phone', 'No test phone number provided.' );
        }
        ...
        self::log( 'Sending test SMS to ' . $phone );

        return self::send_sms_clicksend( $phone, $message, $settings );
    }

    /**
     * Replace tokens like {name} and {cart_url} in the template.
     *
     * @param string $template
     * @param array  $tokens
     * @return string
     */
    protected static function replace_template_tokens( $template, $tokens ) {
        if ( empty( $tokens ) ) {
            return $template;
        }
